...
def place_orders(board,player,actions):
    ...
    print('{0}`s turn. You have {1} points. Where do you use them? Input h for help.'.format(player.name,player.score))
    action = input('--> ')
    #print(action)
    if action=='e':
        if player.score==0:
            ...
            print('Are you sure? You still have {0} points. Input end to end your turn anyway.'.format(player.score))
    elif action == 'end':
        print('-----Next Player-----')
        return actions
    elif action == 'h':
        ...
        if action in option_names:
            location=action
            action = input('How many points? ')
            action=int(action)
            if action<=player.score:
                #print('~~~~~~~~SUCCESSS~~~~~~~~~~')
                player.score-=action
                bid=action
                for ter in options:
                    if ter.name==location:
                        location=ter
                investment=[location,bid,player]
    ...
    return place_orders(board,player,new_actions)
